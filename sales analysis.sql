SELECT * FROM `sales-497411.data_sales.user_events` LIMIT 1000;

-- defining sales funnel and stage views
...
WITH funnel_stages AS (
  ...
  FROM `sales-497411.data_sales.user_events`
...
stage_1_views,
stage_2_carts,
ROUND(stage_2_carts * 100/stage_1_views) AS view_to_cart_rate,

stage_3_checkout,
ROUND(stage_3_checkout * 100/stage_2_carts) AS cart_to_checkout_rate,
...
SELECT traffic_source, views, carts, purchase,
ROUND(carts * 100/views) AS cart_conversion_rate,
ROUND(purchase * 100/views) AS purchase_conversion_rate,
ROUND(purchase * 100/carts) AS carts_to_purchase_conversion_rate,
